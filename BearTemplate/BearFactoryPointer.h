...
class BearFactoryPointer
{
	...
	struct SÑontainer
	{
		SÑontainer():Object(0), Count(1){}
		SÑontainer(void *object, BearGraphicsObjectType objectType):Object(object), ObjectType(objectType), Count(1){}
		~SÑontainer() { Impl::BearRemoverObject::Remove<T>(Object, ObjectType); Object = 0; }
		void *Object;
...
public:
	BearFactoryPointer(void*data, void*CurObj) :m_container(reinterpret_cast<SÑontainer*>(data)), m_CurObject(CurObj), m_CurrentType(Impl::BearTypeManager::GetType<T>()) {  }
public:
	BearFactoryPointer() :m_container(0), m_CurObject(0){}
	BearFactoryPointer(T*c) :m_container(0), m_CurObject(0) { m_container = bear_new< SÑontainer>(reinterpret_cast<T*>(c), Impl::BearTypeManager::GetType<T>()); m_CurrentType = Impl::BearTypeManager::GetType<T>(); }
	inline void clear() 
	{ 
		if (m_container)
		{
	...
	inline BearFactoryPointer<C> cast() const
	{
		...
		if (out)
		{
			...
			BearFactoryPointer<C> result(reinterpret_cast<void*>(const_cast<SÑontainer*>(m_container)), out);
			return result;
		}
		else
		{
	...
	inline void replace(BearFactoryPointer&right)
	{
		...
		if (right.empty())
		{
			right.m_container = m_container =bear_new< SÑontainer>();
		}
		if (empty())
		{
				m_container = bear_new< SÑontainer>();
		}
