...
namespace BearGraphics
{
	...
	public:
		...
		BearFactoryPointer(void*object, BearGraphicsObjectType type) :m_data(0) { m_data = BearCore::bear_new< Data>(object, type); m_CurrentType = Impl::BearTypeManager::GetType<T>(); }
		...
		inline	bool empty()const { if (m_data)return m_data->Object; return m_data == 0; }
		...
		template<typename C>
		inline BearFactoryPointer<C> cast()
		{
			...
			return BearFactoryPointer<C>(out, m_data->ObjectType);
		}
		...
		template<class C>
		BearFactoryPointer(const BearFactoryPointer<C>&right) :m_data(0) { swap(right.cast<C>());; }
		template<>
		BearFactoryPointer<T>(const BearFactoryPointer&right) :m_data(0) { copy(right); }
