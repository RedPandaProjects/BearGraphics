...
namespace BearGraphics
{
	...
	public:
		...
		BearFactoryPointer(T*c) :m_data(0), m_CurObject(0) { m_data = BearCore::bear_new< Data>(reinterpret_cast<T*>(c), Impl::BearTypeManager::GetType<T>()); m_CurrentType = Impl::BearTypeManager::GetType<T>(); }
		inline void clear() 
		{ 
			if (m_data)
			{
		...
		inline void swap(BearFactoryPointer&right)
		{
			...
			BearCore::bear_swap(m_CurObject, right.m_CurObject);
		}
		template<class C>
