...
bsize BearRHI::BearRHITextureUtils::GetSizeWidth(bsize w, BearGraphics::BearTexturePixelFormat format)
{
...
bsize BearRHI::BearRHITextureUtils::GetSizeDepth(bsize w, bsize h, BearGraphics::BearTexturePixelFormat format)
{
	if (isCompressor(format))
	{
...
bsize BearRHI::BearRHITextureUtils::GetSizePixel(BearGraphics::BearTexturePixelFormat format)
{
	switch (format)
	{
...
void BearRHI::BearRHITextureUtils::Append(uint8*dst, bsize w_dst, bsize h_dst, bsize x_dst, bsize y_dst, uint8*src, bsize w_src, bsize h_src, BearCore::BearVector4<bsize> squard_src, BearGraphics::BearTexturePixelFormat dst_format, BearGraphics::BearTexturePixelFormat src_format)
{
...
bsize BearRHI::BearRHITextureUtils::GetCountMips(bsize w, bsize h)
{
	bsize max_size = BearCore::bear_max(w, h);
	return log2(static_cast<double>(max_size))+1;
}

bsize BearRHI::BearRHITextureUtils::GetMip(bsize w, bsize level)
{
	bsize mip = static_cast<bsize>((w ) / pow(2, static_cast<double>(level)));
	return BearCore::bear_max(mip, bsize(1));
}

uint8 * BearRHI::BearRHITextureUtils::GetPixelUint8(bsize x, bsize y, bsize w, bsize comps, bsize comp_id, uint8 * data)
{
	return  &data[(x + (y * w))*comps+comp_id];
}

float * BearRHI::BearRHITextureUtils::GetPixelFloat(bsize x, bsize y, bsize w, bsize comps, bsize comp_id, uint8 * data)
{
	return (float*)&data[y*w*comps*4+x*4*comps+comp_id*4];
}

bool BearRHI::BearRHITextureUtils::isFloatPixel(BearGraphics::BearTexturePixelFormat format)
{
	switch (format)
	{
	case BearGraphics::TPF_R8:
...
bool BearRHI::BearRHITextureUtils::isCompressor(BearGraphics::BearTexturePixelFormat format)
{
	switch (format)
	{
	case BearGraphics::TPF_R8:
...
void BearRHI::BearRHITextureUtils::Fill(uint8 * data, bsize w, bsize h, bsize mip, const BearCore::BearColor & color, BearGraphics::BearTexturePixelFormat format)
{
...
bsize BearRHI::BearRHITextureUtils::GetSizeInMemory(bsize w, bsize h, bsize mips, BearGraphics::BearTexturePixelFormat format)
{
	bsize result = 0;
	if (!isCompressor(format))
	{
...
void BearRHI::BearRHITextureUtils::Convert(BearGraphics::BearTexturePixelFormat dst_format, BearGraphics::BearTexturePixelFormat src_format, uint8 * dst, uint8 * src, bsize w, bsize h)
{
...
bsize BearRHI::BearRHITextureUtils::GetCountBlock(bsize w)
{
	return (w + 3) / 4;
}

bsize BearRHI::BearRHITextureUtils::GetSizeBlock(BearGraphics::BearTexturePixelFormat format)
{
	switch (format)
	{
	case BearGraphics::TPF_DXT_1:
...
uint8 BearRHI::BearRHITextureUtils::GetCountComp(BearGraphics::BearTexturePixelFormat format)
{
	switch (format)
	{
	case BearGraphics::TPF_R8:
...
void BearRHI::BearRHITextureUtils::GenerateMip(uint8 * dst, uint8 * src, bsize w_src, bsize h_src, BearGraphics::BearTexturePixelFormat format)
{
	if (isFloatPixel(format))
	{
...
void BearRHI::BearRHITextureUtils::Scale(uint8 * dst, bsize w_dst, bsize h_dst, uint8 * src, bsize w_src, bsize h_src, BearGraphics::BearTexturePixelFormat format)
{
	if (isFloatPixel(format))
	{
...
uint8 * BearRHI::BearRHITextureUtils::GetImage(uint8 * data, bsize w, bsize h, bsize mips, bsize depth, bsize mip, BearGraphics::BearTexturePixelFormat format)
{
	return data + BearRHI::BearRHITextureUtils::GetSizeInMemory(w, h, mips, format)*depth + BearRHI::BearRHITextureUtils::GetSizeInMemory(w, h, mip, format);
}

void BearRHI::BearRHITextureUtils::ScaleFloat(uint8 * dst, bsize w_dst, bsize h_dst, uint8 * src, bsize w_src, bsize h_src, uint8 comp)
{
	stbir_resize_float(reinterpret_cast<float*>(src), w_src, h_src, 0, reinterpret_cast<float*>(dst), w_dst, h_dst, 0, comp);
}

void BearRHI::BearRHITextureUtils::ScaleUint8(uint8 * dst, bsize w_dst, bsize h_dst, uint8 * src, bsize w_src, bsize h_src, uint8 comp)
{
...
void BearRHI::BearRHITextureUtils::GenerateMipFloat(uint8 * dst, uint8 * src, bsize w_src, bsize h_src, bsize comps)
{
	ScaleFloat(dst, w_src / 2, h_src / 2, src, w_src, h_src, comps);
}

void BearRHI::BearRHITextureUtils::GenerateMipUint8(uint8 * dst, uint8 * src, bsize w_src, bsize h_src, bsize comps)
{
	ScaleUint8(dst, w_src / 2, h_src / 2, src, w_src, h_src, comps);
}

void BearRHI::BearRHITextureUtils::FloatPixelToUint8(uint8 * dst, float * src, uint8 comp_dst, uint8 comp_src)
{
...
void BearRHI::BearRHITextureUtils::FloatPixelToFloat(float * dst, float * src, uint8 comp_dst, uint8 comp_src)
{
	for (uint8 i = 0; i < comp_dst; i++)
	{
		if (i < comp_src)
...
void BearRHI::BearRHITextureUtils::Uint8PixelToFloat(float * dst, uint8 * src, uint8 comp_dst, uint8 comp_src)
{
	for (uint8 i = 0; i < comp_dst; i++)
	{
		if (i < comp_src)
...
void BearRHI::BearRHITextureUtils::Uint8PixelToUint8(uint8 * dst, uint8 * src, uint8 comp_dst, uint8 comp_src)
{
	for (uint8 i = 0; i < comp_dst; i++)
	{
		if (i < comp_src)
...
void BearRHI::BearRHITextureUtils::FloatPixelToHalf4(uint16 * dst, float * src, uint8 comp_src)
{
	for (uint8 i = 0; i < 4; i++)
	{
		if (i < comp_src)
...
void BearRHI::BearRHITextureUtils::Uint8PixelToHalf4(uint16 * dst, uint8 * src, uint8 comp_src)
{
	for (uint8 i = 0; i < 4; i++)
	{
		if (i < comp_src)
...
void BearRHI::BearRHITextureUtils::SwapRB(uint32 & color)
{
...
void BearRHI::BearRHITextureUtils::Uint8ToHalf4(uint8 * dst, uint8 * src, bsize w, bsize h, uint8 comp_src)
{
...
void BearRHI::BearRHITextureUtils::Uint8ToFloat(uint8 * dst, uint8 * src, bsize w, bsize h, uint8 comp_dst, uint8 comp_src)
{
	for (bsize i = 0; i < w*h; i++)
	{
...
void BearRHI::BearRHITextureUtils::Uint8ToUint8(uint8 * dst, uint8 * src, bsize w, bsize h, uint8 comp_dst, uint8 comp_src)
{
	for (bsize i = 0; i < w*h; i++)
	{
...
void BearRHI::BearRHITextureUtils::Uint8ToCompressor(uint8 * dst, uint8 * src, bsize w, bsize h, BearGraphics::BearTexturePixelFormat compressor, uint8 comp_src)
{
...
void BearRHI::BearRHITextureUtils::FloatToHalf4(uint8 * dst, uint8 * src, bsize w, bsize h, uint8 comp_src)
{
	for (bsize i = 0; i < w*h; i++)
	{
		Uint8PixelToHalf4((uint16*)dst, src, comp_src);
...
void BearRHI::BearRHITextureUtils::FloatToUint8(uint8 * dst, uint8 * src, bsize w, bsize h, uint8 comp_dst, uint8 comp_src)
{
	for (bsize i = 0; i < w*h; i++)
	{
...
void BearRHI::BearRHITextureUtils::FloatToFloat(uint8 * dst, uint8 * src, bsize w, bsize h, uint8 comp_dst, uint8 comp_src)
{
	for (bsize i = 0; i < w*h; i++)
	{
...
void BearRHI::BearRHITextureUtils::FloatToCompressor(uint8 * dst, uint8 * src, bsize w, bsize h, BearGraphics::BearTexturePixelFormat compressor, uint8 comp_src)
{
	uint8*in = (uint8*)StartCompressor(compressor, w, h);
	switch (compressor)
	{
...
void BearRHI::BearRHITextureUtils::CompressorToCompressor(uint8 * dst, uint8 * src, bsize w, bsize h, BearGraphics::BearTexturePixelFormat compressor_dst, BearGraphics::BearTexturePixelFormat compressor_src)
{
...
void BearRHI::BearRHITextureUtils::CompressorToUint8(uint8 * dst, uint8 * src, bsize w, bsize h, uint8 comp_dst, BearGraphics::BearTexturePixelFormat compressor)
{
...
void BearRHI::BearRHITextureUtils::CompressorToFloat(uint8 * dst, uint8 * src, bsize w, bsize h, uint8 comp_dst, BearGraphics::BearTexturePixelFormat compressor)
{
	uint8*in = (uint8*)StartDecompressor(compressor, w, h, src);
	switch (compressor)
	{
...
void * BearRHI::BearRHITextureUtils::StartCompressor(BearGraphics::BearTexturePixelFormat compressor, bsize w, bsize h)
{
	switch (compressor)
	{
	case BearGraphics::TPF_DXT_1:
...
void BearRHI::BearRHITextureUtils::EndCompressor(BearGraphics::BearTexturePixelFormat compressor, bsize w, bsize h, void*in, void*out)
{
	switch (compressor)
	{
	case BearGraphics::TPF_DXT_1:
...
void * BearRHI::BearRHITextureUtils::StartDecompressor(BearGraphics::BearTexturePixelFormat compressor, bsize w, bsize h, void * in)
{
	switch (compressor)
	{
	case BearGraphics::TPF_DXT_1_alpha:
...
void BearRHI::BearRHITextureUtils::EndDecompressor(void * in)
{
	BearCore::bear_free(in);
}
