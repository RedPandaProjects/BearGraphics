...
void BearGraphics::BearImage::Create(bsize w, bsize h, bsize mip, bsize depth, BearTexturePixelFormat px)
{
	...
	{
		BEAR_FATALERROR(m_w%4==0&&m_h%4==0, TEXT("Некоректный размер для формата компресси!"));
	}
	m_w = w;
	m_h = h;
	m_px = px;
	BEAR_FATALERROR(m_depth, TEXT("Глубина неможет равняться нулю!"));
...
void BearGraphics::BearImage::Append(bsize x, bsize y, const BearImage & img, bsize x_src, bsize y_src, bsize w_src, bsize h_src, bsize dst_depth, bsize src_depth)
{
	if (Empty() || img.Empty())
		return;
	BEAR_FATALERROR(m_w >= x + w_src && m_h >= y + h_src, TEXT("Незвохможно склеить текстуру из-за маленького размера полотна."));
	BEAR_FATALERROR(img.m_w >= x_src + w_src && img.m_h >= y_src + h_src, TEXT("Незвохможно склеить текстуру из-за маленького размера полотна."));
	BEAR_FATALERROR(m_depth > dst_depth, TEXT("Значение глубены [%llu] вышло за пределы [%llu] масива"), dst_depth, m_depth);
	BEAR_FATALERROR(m_depth > src_depth, TEXT("Значение глубены [%llu] вышло за пределы [%llu] масива"), src_depth, m_depth);
